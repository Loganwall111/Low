...
//
//  1.9.221 (port): day/night vaults retuned to the 2026-09-11 references
...
// 1.9.221 (port) -- day revamped: clear vivid mid-blue zenith falling to
...
vec3 mcsm_storm_dome(float up, float p) {
    ...
    // 1.9.221 (port) -- 5.0 re-keyed to the 2026-09-11 turquoise deck
    ...
    // 1.9.221 (port) -- phase 6 is the FOUR-COLOR APOCALYPTIC SUNSET SPLIT
...
void main() {
    ...
    // 1.9.221 (port) -- THE INFINITE SKYBOX BLOB. The glare is a separate
